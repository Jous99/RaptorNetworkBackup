...
void SoftwareKeyboard::InitializeFrontendKeyboard() {
    if (is_background) {
        ...
        u32 max_text_length = swkbd_config_common.max_text_length > 0 &&
                                      swkbd_config_common.max_text_length <= DEFAULT_MAX_TEXT_LENGTH
                                  ? swkbd_config_common.max_text_length
                                  : DEFAULT_MAX_TEXT_LENGTH;

        if (header_text == u"Please input the Course ID or Maker ID you want to search for.") {
            max_text_length = 11;
        }
        ...
        frontend.InitializeKeyboard(
            ...
            [this, header_text](SwkbdResult result, std::u16string submitted_text) {
                if (header_text ==
                    u"Please input the Course ID or Maker ID you want to search for.") {
                    auto old = std::move(submitted_text);
                    submitted_text.clear();
                    for (const auto& c : old) {
                        if (c != '-' && c != ' ') {
                            submitted_text.push_back(static_cast<char16_t>(std::toupper(c)));
                        }
                    }
                }
                SubmitTextNormal(result, std::move(submitted_text));
            },
            {});
    }
}
