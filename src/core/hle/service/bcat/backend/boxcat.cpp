...
constexpr char BOXCAT_PATHNAME_LAUNCHPARAM[] = "/v1/citrus/launchparam/{:016X}";

// HTTP status codes for Boxcat
enum class ResponseStatus {
    Ok = 200,               ///< Operation completed successfully.
    BadClientVersion = 400, ///< The Boxcat-Client-Version doesn't match the server.
    ...
    DownloadResult DownloadInternal(const std::string& resolved_path, u32 timeout_seconds,
        ...
        httplib::Headers headers{
            {std::string("Authorization"),
             fmt::format(
                 "Bearer {}",
                 Core::System::GetInstance().OnlineInitiator().LoadIdTokenApp("bcat").value_or(
                     ""))},
            {std::string("Game-Assets-API-Version"), std::string(Common::g_build_version)},
            {std::string("Boxcat-Client-Type"), std::string("RaptorCitrus")},
