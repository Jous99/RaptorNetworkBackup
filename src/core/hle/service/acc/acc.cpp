...
public:
    explicit IAsyncContext(Core::System& system, std::function<bool()> work)
        : ServiceFramework{system, "IAsyncContext"}, granter(system, std::move(work)) {
        // clang-format off
        static const FunctionInfo functions[] = {
...
private:
    void GetSystemEvent(Kernel::HLERequestContext& ctx) {
        ...
        rb.Push(RESULT_SUCCESS);
        rb.PushCopyObjects(granter.GetSystemEvent()->GetReadableEvent());
    }
    ...
    AsyncOperation granter;
};

class IAuthorizationRequest final : public ServiceFramework<IAuthorizationRequest> {
public:
    explicit IAuthorizationRequest(Core::System& system_, std::string scopes, std::string state,
                                   std::string nonce)
        : ServiceFramework{system_, "IAuthorizationRequest"}, state(state),
          async(std::make_shared<IAsyncContext>(
              system_, [this, &system_, scopes = std::move(scopes), state = std::move(state),
                        nonce = std::move(nonce)] {
                  const u64 title_id = system_.CurrentProcess()->GetTitleID();
                  auto& online_initiator = system_.OnlineInitiator();
                  online_initiator.StartOnlineSession(title_id);

                  auto token = online_initiator.LoadIdTokenApp("auth_connector");
                  if (!token) {
                      return false;
                  }

                  const auto& url = online_initiator.ConnectorUrl();
                  httplib::SSLClient client(url);
                  const auto response =
                      client.Post("/api/v1/request/standard",
                                  {
                                      {"R-Auth-Scopes", scopes},
                                      {"R-Auth-State", state},
                                      {"R-Auth-Nonce", nonce},
                                      {"Authorization", fmt::format("Bearer {}", token.value())},
                                  },
                                  "", "");
                  ...
                  try {
                      ...
                      out_code = json.at("token_code").get<std::string>();
                      out_id = json.at("token_id").get<std::string>();
                      return true;
                  } catch (...) {
                  }
...
private:
    void InvokeWithoutInteractionAsync(Kernel::HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "called");
    ...
    void IsAuthorized(Kernel::HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "called");
        ...
        std::lock_guard guard{mutex};
        rb.Push<u32>((out_code.empty() || out_id.empty()) ? 0 : 1);
    }

    void GetAuthorizationCode(Kernel::HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "called");
        ...
        std::lock_guard guard{mutex};
        ctx.WriteBuffer(out_code);
        rb.Push<u32>(static_cast<u32>(out_code.size()));
    }

    void GetIdToken(Kernel::HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "called");
        ...
        std::lock_guard guard{mutex};
        ctx.WriteBuffer(out_id);
        rb.Push<u32>(static_cast<u32>(out_id.size()));
    }
    ...
    void GetState(Kernel::HLERequestContext& ctx) {
        ...
        IPC::ResponseBuilder rb{ctx, 2};
        rb.Push(RESULT_SUCCESS);
        ctx.WriteBuffer(state.c_str(), state.size() + 1);
    }

    std::string state;

    std::shared_ptr<IAsyncContext> async;
...
private:
    ...
    void CreateAuthorizationRequest(Kernel::HLERequestContext& ctx) {
        LOG_DEBUG(Service_ACC, "called");
        ...
        auto scopes = Common::StringFromFixedZeroTerminatedBuffer(
            reinterpret_cast<const char*>(buffer.data()), 0x100);
        auto state = Common::StringFromFixedZeroTerminatedBuffer(
            reinterpret_cast<const char*>(buffer.data()) + 0x100, 0x80);
        auto nonce = Common::StringFromFixedZeroTerminatedBuffer(
            reinterpret_cast<const char*>(buffer.data()) + 0x180, 0x80);

        IPC::ResponseBuilder rb{ctx, 2, 0, 1};
        rb.Push(RESULT_SUCCESS);
        rb.PushIpcInterface<IAuthorizationRequest>(system, std::move(scopes), std::move(state),
                                                   std::move(nonce));
    }
    ...
    std::string token;
    u64 id = Core::System::GetInstance().OnlineInitiator().GetAccountId();
};
