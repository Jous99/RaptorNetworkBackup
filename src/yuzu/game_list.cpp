...
GameList::GameList(FileSys::VirtualFilesystem vfs, FileSys::ManualContentProvider* provider,
                   Core::OnlineInitiator& online_initiator, GMainWindow* parent)
    : QWidget{parent}, vfs(std::move(vfs)), provider(provider), online_initiator(online_initiator) {
...
void GameList::LoadCompatibilityList() {
    ...
    const auto json_titles = nlohmann::json::parse(response_titles->body);
    for (auto iter = json_titles.begin(); iter != json_titles.end(); ++iter) {
        compatibility_list.insert_or_assign(std::stoull(iter.key(), nullptr, 0x10),
                                            iter->get<unsigned int>());
    }
}

bool GameList::IsGameSupportedOnline(u64 title_id) {
    const auto iter = compatibility_list.find(title_id);
    return iter != compatibility_list.end() && iter->second == 1;
}
...
void GameList::PopulateAsync(QVector<UISettings::GameDir>& game_dirs) {
    ...
    item_model->insertColumns(0, UISettings::values.show_add_ons ? COLUMN_COUNT : COLUMN_COUNT - 1);
    item_model->setHeaderData(COLUMN_NAME, Qt::Horizontal, tr("Name"));
    item_model->setHeaderData(COLUMN_COMPATIBILITY, Qt::Horizontal, tr("Online Status"));
