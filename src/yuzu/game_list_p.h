...
public:
    ...
    explicit GameListItemCompat(unsigned int compatibility) {
        setData(type(), TypeRole);
        ...
        static const std::map<unsigned int, CompatStatus> status_data = {
            {
                0,
                {QStringLiteral("#ff0000"), QT_TR_NOOP("Not Supported"),
                 QT_TR_NOOP(
                     "Game does not have any supported online functionality on Raptor Network.")},
            },
            {
                1,
                {QStringLiteral("#47d35c"), QT_TR_NOOP("Supported"),
                 QT_TR_NOOP("Game is supported for online connectivity using Raptor Network.")},
            },
        };

        auto iterator = status_data.find(compatibility);
        if (iterator == status_data.end()) {
            LOG_WARNING(Frontend, "Invalid compatibility number {}", compatibility);
            return;
        }
