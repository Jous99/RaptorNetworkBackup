...
void GMainWindow::ShowTelemetryCallout() {
    Settings::values.enable_telemetry = true;
}

void GMainWindow::ShowRaptorLoginCallout() {
    if (!Settings::values.raptor_token.empty()) {
        return;
    }

    const QString message =
        tr("You have not yet logged into Raptor Network, or you were logged out due to inactivity. "
           "Would you like to login now? You must do this to play games online.");
    if (QMessageBox::question(this, tr("Login to Raptor Network"), message) == QMessageBox::Yes) {
        emit StartLoginExternal();
    }
}
...
GMainWindow::GMainWindow()
    ...
    LOG_INFO(Frontend, "RaptorCitrus Version: {}", Common::g_build_version);
#ifdef ARCHITECTURE_x86_64
    ...
    connect(this, &GMainWindow::StartLoginExternal, online_status_monitor,
            &OnlineStatusMonitor::OnStartLoginExternal);

    // Show one-time "callout" messages to the user
    ShowTelemetryCallout();
    ShowRaptorLoginCallout();
...
void GMainWindow::InitializeWidgets() {
    ...
    game_list = new GameList(vfs, provider.get(), online_initiator, this);
    ...
    connect(online_status_monitor, &OnlineStatusMonitor::SaveConfig, this,
            [this] { config->Save(); });
...
bool GMainWindow::LoadROM(const QString& filename, std::size_t program_index) {
    ...
    const Core::System::ResultStatus result{
        system.Load(*render_window, online_initiator, filename.toStdString(), program_index)};
    ...
    if (!game_list->IsGameSupportedOnline(title_id)) {
        QMessageBox::warning(
            this, tr("Unsupported Game Online"),
            tr("The game you are trying to launch does not have supported online functionality "
               "with Raptor Network."));
        // TODO: Add additional help text.
    }
    ...
    const auto& fsc{Core::System::GetInstance().GetFileSystemController()};
    if (title_id == 0x01009B90006DC000ull &&
        fsc.OpenRomFS(0x0100000000000802ull, FileSys::StorageId::NandSystem,
                      FileSys::ContentRecordType::Data)
            .Failed() &&
        FileSys::SystemArchive::GetImportedSystemArchive(fsc.GetSysdataImportedDirectory(),
                                                         0x0100000000000802ull) == nullptr) {
        QMessageBox::critical(
            this, tr("Missing Mii System Archives"),
            tr("The game you are trying to launch (Super Mario Maker 2) requires "
               "a real copy of the Mii system archives. Without them, the game "
               "will not work properly when trying to play online.<br>Please "
               "follow <a href='https://yuzu-emu.org/help/quickstart/'>the"
               " yuzu quickstart guide</a> to dump these files from your switch."));
        system.Shutdown();
        return false;
    }
...
void GMainWindow::OnGameListNavigateToGamedbEntry(u64, const CompatibilityList&) {}

void GMainWindow::OnClearImportedSysdata() {
    const auto path =
        Common::FS::GetUserPath(Common::FS::UserPath::SysDataDir) + "imported" DIR_SEP;
...
void GMainWindow::UpdateWindowTitle(const std::string& title_name,
    ...
    if (title_name.empty()) {
        ...
        setWindowTitle(QString::fromStdString(
            fmt::format(fmt.empty() ? "ERROR" : fmt, std::string{}, std::string{}, std::string{},
                        std::string{}, date, std::string{})));
    } else {
        const auto fmt = std::string(Common::g_title_bar_format_running);
        setWindowTitle(QString::fromStdString(
            fmt::format(fmt.empty() ? "ERROR" : fmt, std::string{}, std::string{}, std::string{},
                        title_name, date, std::string{}, title_version)));
    }
}
